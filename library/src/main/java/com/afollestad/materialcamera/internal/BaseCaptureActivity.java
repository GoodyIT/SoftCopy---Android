...
public abstract class BaseCaptureActivity extends AppCompatActivity
  ...
  @Override
  public void onCustomShowStillshot(byte[] data) {
//    Fragment frag =
//            StillshotPreviewFragment.customInstance(
//                    data, allowRetry(), getIntent().getIntExtra(CameraIntentKey.PRIMARY_COLOR, 0));
//    getFragmentManager().beginTransaction().replace(R.id.container, frag).commit();
    if (data != null) {
      String imageData = null;
      try {
        imageData = new String(data, "UTF-8");
      ...
      setResult(
              ...
              getIntent()
                      ...
                      .setDataAndType(Uri.parse(imageData), useStillshot() ? "image/jpeg" : "video/mp4"));
    }
  ...
  @Override
  public final void useCustomMedia(byte[] data) {
    if (data != null) {
      try {
        setResult(
                Activity.RESULT_OK,
                getIntent()
                        .putExtra(MaterialCamera.STATUS_EXTRA, MaterialCamera.STATUS_RECORDED)
                        .setDataAndType(Uri.parse(new String(data, "UTF-8")), useStillshot() ? "image/jpeg" : "video/mp4"));
      } catch (UnsupportedEncodingException e) {
        e.printStackTrace();
      }
    }
